...
export class AIService {
  private static agentRouterToken = process.env.ANTHROPIC_AUTH_TOKEN || "sk-elSrFN5fD7HXk5ggOoxieT3ZOIsnvrl8gVrNxaN3KU0UwlG9";
  private static agentRouterBaseUrl = process.env.ANTHROPIC_BASE_URL || "https://agentrouter.org";
  private static agentRouterModel = process.env.CLAUDE_CODE_SUBAGENT_MODEL || process.env.ANTHROPIC_DEFAULT_OPUS_MODEL || "claude-opus-4-8";

  private static agentRouterClient = new Anthropic({
  ...
  static async generateResponse(
    ...
    for (const provider of chain) {
      try {
        const hasKey = this.hasApiKey(provider.keyName);
        if (!hasKey) {
  ...
  private static hasApiKey(name: string): boolean {
    const key = name === 'AgentRouter' ? (process.env.ANTHROPIC_AUTH_TOKEN || this.agentRouterToken)
  ...
  private static async callAgentRouter(system: string, messages: Anthropic.MessageParam[], tools: any[]): Promise<AIProviderResponse> {
    const response = await this.agentRouterClient.messages.create({
      model: this.agentRouterModel,
      max_tokens: 4096,
      system,
      messages,
      tools
  ...
  private static async callGroq(system: string, messages: Anthropic.MessageParam[], tools: any[]): Promise<AIProviderResponse> {
    ...
    try {
      ...
      console.warn('⚠️ Groq call error, retrying without tools:', toolErr.message);
