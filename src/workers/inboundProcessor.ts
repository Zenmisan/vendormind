...
const worker = new Worker<InboundMessageJob>(
  ...
  async (job) => {
    try {
      ...
      if (hasOpenAI) {
        ...
        if (totalProducts > 0) {
          ...
          if (progress < 80.0) {
            ...
            // Only notify each customer once per 6h to avoid spamming on reconnect floods
            const gateKey = `catalog_gate_notified:${vendorId}:${customerPhone}`;
            const alreadyNotified = !(await redisConnection.set(gateKey, '1', 'EX', 21600, 'NX'));
            if (!alreadyNotified) {
      ...
      // 4.1. Auto-provision BMONI CNGN Smart Wallet for Customer asynchronously (non-blocking)
      if (!customer.bmoniUserId || !customer.bmoniSmartWalletId) {
        const targetCustomerId = customer.id;
        const targetName = customer.name || `Customer ${customerPhone}`;
        const targetPhone = customerPhone;
        
        BmoniService.provisionUserStack(
          targetName,
          `${targetPhone.replace(/[^\d]/g, '')}@wa.vendormind.app`,
          targetPhone
        ).then(async (bmoniStack) => {
          ...
          console.log(`🏦 Auto-provisioned BMONI Smart Wallet for Customer ${targetCustomerId}: ${bmoniStack.depositBankName} ${bmoniStack.depositAccountNumber}`);
        }).catch((err) => {
          console.error(`⚠️ Async BMONI wallet provisioning notice for Customer ${targetCustomerId}:`, err.message);
        });
      }
