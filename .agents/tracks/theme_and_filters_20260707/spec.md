...
3. **Generic Browser Favicon**: The application has no icon defined in `index.html`, falling back to browser defaults instead of representing the brand.
...
3. **Favicon integration**: Add `<link rel="icon">` targeting `/logo-dark.png` to the page `<head>`.
