...
3. **Checkout Amount Units (Kobo vs Naira)**: The Nomba Checkout API expects the transaction amount in kobo, but the application was passing it in Naira, resulting in users being charged 100x less than expected.
...
3. **Amount Scaling**: Update checkout creation calls to multiply NGN amounts by 100 to convert from Naira to Kobo.
